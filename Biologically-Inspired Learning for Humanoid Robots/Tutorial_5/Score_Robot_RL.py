...
class Central:
    ...
    def central_execute(self):
        ...
        ## Reinforcement Learning begins from here!!!!!!!!!! ##
        ...
        R = np.zeros((72, 3))            # Reward function (Because we have only 72 states, and in each state we can do three actions)
        ...
        Q_table = np.zeros((72, 3))       # The Q value, but in this case we do not use Q-Learning
        Qk_table = 10*np.ones((72, 3))       # The Q value for next state, set as we want, but not the same as Q_table
        n_episodes = 100
        max_state = 8
        accumulate_reward = 0
        scores_list = []
        ...
        for i_episode in range(1, n_episodes+1):
            # Check if the goalie should be moved before training
            while goalie2bemoved:
                new_pos = (self.x - 31)/145 * 6
                new_pos = int(floor(new_pos))
                if (new_pos > 5):
                    new_pos = 5
                elif (new_pos < 0):
                    new_pos = 0 
            ...
            for t in range(max_state - 1):
                ...
                if action != 2:
                    ...
                    temp = raw_input("Is the robot falling down? [y/n]：")            # For python 2.7
                    # temp = input("Is the robot falling down? [y/n]：")          # For python 3.x
                    while not temp.isalpha():
                        print("Sorry, the entry is invalid. ")
                        temp = raw_input("Please entry if the robot is falling down once again [y/n]: ")          # For python 2.7
                        # temp = input("Please entry if the robot is falling down once again [y/n]: ")            # For python 3.x
                    if temp == 'y':
                        R[state_index, action] = -20
                        accumulate_reward += -20
                        done = True          
                    ...
                    for i in range(5):
                        ...
                        self.kick()
                        print(state_index, action)
                        temp = raw_input("Please evaluate the goal performance：")            # For python 2.7
                        # temp = input("Please evaluate the goal performance：")          # For python 3.x
                        while not temp.lstrip('-').isdigit():
                            print("Sorry, the entry is invalid. ")
                            temp = raw_input("Please entry your reward once again: ")         # For python 2.7
                            # temp = input("Please entry your reward once again: ")           # For python 3.x
                        reward = int(temp)
                        # R[state_index, action] = reward
                        accumulate_reward_inLoop += reward
                        self.kick_ready()
                        rate = rospy.sleep(5)
                        self.joint_moveto(np.clip(next_state, -0.379472, 0.790477), "LHipRoll", 0.05)
                        rate = rospy.sleep(10)
                        if i == 4:
                            if (accumulate_reward_inLoop / 5) >= 8:
                                print('Congratulations! You find a good policy. Please move the goal keeper to another postion and train again')
                                R[state_index, 2] = (accumulate_reward_inLoop / 5)
                                goalie2bemoved = True
                                moved_time += 1
                                done = True

                            else:
                                print('Sorry, the scoring is not good. We should train once again')
                                R[state_index, 2] = -2
                                done = True
            ...
            # scores_list.append(accumulate_reward)

            # Value iteration
            Q_table = np.zeros((72, 3))       # The Q value, but in this case we do not use Q-Learning
            Qk_table = 10*np.ones((72, 3))       # The Q value for next state, set as we want, but not the same as Q_table
            while np.linalg.norm(Qk_table - Q_table) > 0:
                Q_table = Qk_table
                Q_max = np.max(Q_table,axis=1)
                Q_max = Q_max.reshape(72, 1)
                Qk_table = R + np.hstack((gamma*np.dot(Pmove[0, :, :], Q_max), gamma*np.dot(Pmove[1, :, :], Q_max), gamma*np.dot(Pmove[2, :, :], Q_max)))

            # End the training and save the Q_table
            if moved_time == 6: # When you want to do the total training, use this one 
            #if moved_time == 1:     # When you only want to train the process with the goalie in a certain position, then use this one and move the goalie to your desire position
                np.savetxt('Qtable.txt', Qk_table, fmt='%f')
                print('Congratulations! You have finished the whole training!')
                break

             ## Reinforcement Learning ends here!!!!!!!!!! ##


        # Plot the accumulate reward
        # fig = plt.figure()
        # ax = fig.add_subplot(111)
        # plt.plot(np.arange(1, len(scores_list)+1), scores_list)
        # plt.title('Averaged Score over Last 100 Eposides')
